...
class FirestoreDatabase {
  ...
  Future<String?> getFacts(String service) async {
    ...
    try {
      ...
      if (ref.docs.isEmpty) {
        ...
        final List<String> fact = ref.docs
            .map(
  ...
  Future<bool> shouldFetchFromAPI(String api) async {
    ...
    try {
      final storedData = jsonDecode(jsonData!);
      final dateData = jsonDecode(jsonDateData!);
  ...
  Stream<ComparisonModel> getDatabaseAnswer(List<AnswersSelected> answersSelected) {
  final String docID = answersSelected.toString().hashCode.toString();
  final Stream<DocumentSnapshot<Object?>> ref = databaseComparison.doc(docID).snapshots();
  // Use snapshots() instead of get() to get a Stream<DocumentSnapshot> object
  return ref.map((DocumentSnapshot<Object?> doc) => ComparisonModel.fromJson(doc.data()! as Map<String, dynamic>));
  ...
  Future<void> writeCloudData(CloudData cloudData) async {
    ...
    try {
      ...
      final DocumentReference ref = servicesCollection.doc(serviceName);
